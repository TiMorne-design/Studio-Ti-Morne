/**
 * Hook personnalisé pour les contrôles de caméra avec souris
...
export default function useCameraControls(cameraRef, splineRef) {
  ...
  const config = {
    ...
    maxPositionOffset: 5,      // Décalage maximum de la position
  ...
  const startAnimationLoop = useCallback(() => {
    ...
    const animate = () => {
      if (cameraRef.current && controlsEnabled && !isAfterButtonClick.current) {
        ...
        if (isOnTerrace.current && 
            currentPos.z <= limits.doorTrigger + 500 && 
          ...
          if (splineRef.current) {
            // Marquer l'ouverture comme automatique
            if (!window.__automaticDoorOpening) {
              ...
              // Émettre l'événement mouseUp sur l'objet PORTE_OUVERT
              try {
    ...
    animationFrameRef.current = requestAnimationFrame(animate);
  }, [controlsEnabled, config.smoothFactor]);
  ...
  }, [controlsEnabled, invertMovementDirection, config.scrollSpeed]);
  
  /**
   * Gère le mouvement de la souris pour orienter la caméra
   * IMPORTANT: Cette fonction est désormais exclusivement pour les événements souris
   * Les événements tactiles sont gérés séparément dans TouchControls.jsx
   * @param {MouseEvent} e - Événement de mouvement de souris
   */
  const handleMouseMove = useCallback((e) => {
    if (!cameraRef.current || !controlsEnabled || isAfterButtonClick.current) return;
    
    // Détecter si l'événement vient d'un appareil tactile et le traiter différemment
    const isTouchEvent = e.isTouchEvent === true || e.type === 'touchmove';
    
    // Traitement spécifique pour les événements tactiles
    if (isTouchEvent) {
      // Utiliser directement les coordonnées normalisées déjà fournies par TouchControls
      const normalizedX = e.normalizedX || 0;
      const normalizedY = e.normalizedY || 0;
      
      // Base de rotation selon la direction du mouvement
      const baseAngle = movementDirection.current > 0 ? 0 : Math.PI;
      
      // Pour les événements tactiles, appliquer directement sans inverser la direction
      targetRotation.current.y = baseAngle + (normalizedX * config.maxSideRotation);
      
      // Rotation verticale adaptée à la direction
      if (movementDirection.current > 0) {
        targetRotation.current.x = -normalizedY * config.maxVerticalAngle;
      } else {
        targetRotation.current.x = normalizedY * config.maxVerticalAngle;
      }
    ...
    // À partir d'ici, c'est un événement souris standard
    
    // Vérifier si on est sur la terrasse et si on doit ignorer l'événement souris
    if (isOnTerrace.current && !hasPerformedFirstTurn.current) {
      return; // Ignorer les rotations sur la terrasse avant le premier demi-tour
    }
    
    // Normaliser la position de la souris entre -1 et 1
    const x = e.normalizedX !== undefined ? e.normalizedX : (e.clientX / window.innerWidth) * 2 - 1;
    const y = e.normalizedY !== undefined ? e.normalizedY : (e.clientY / window.innerHeight) * 2 - 1;
    ...
    // Calcul des rotations - pour la souris, inverser la direction
  ...
  /**
   * Réactive les contrôles sans restaurer la position précédente
   * @returns {Boolean} - true si les contrôles ont été réactivés
   */
  ...
  return {
    ...
    // État
    isControlsEnabled: controlsEnabled,  // Fournir la variable d'état directement
